...
import { Play, Pause, Maximize2, Calendar, TrendingUp, Users, BarChart3 } from 'lucide-react';
      ...
      <div className="container mx-auto px-6">
        ...
        <div className="text-center mb-16 animate-on-scroll">
          ...
          <p className="text-xl text-muted-foreground max-w-2xl mx-auto">
            Watch how JAM transforms your product catalog into high-converting social ads in seconds
          </p>
        </div>
          ...
          <div className="bg-card rounded-3xl border border-border shadow-2xl overflow-hidden animate-on-scroll">
            ...
            <div className="bg-popover border-b border-border p-4">
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm text-muted-foreground">jam.marketing/dashboard</span>
                <Button
                  variant="ghost"
                  size="sm"
              ...
              {activeTab === 'create' && (
                <div className="space-y-6 animate-fade-in">
                  <div className="grid lg:grid-cols-2 gap-8">
                    ...
                    {/* Right - Ad Preview */}
                    <div>
                      <h3 className="text-lg font-semibold mb-4">Generated Ad Preview</h3>
                      <div className="bg-popover rounded-xl p-6 border border-border">
                        ...
                        <p className="font-medium mb-2">✨ Limited Time: 30% OFF!</p>
                        <p className="text-sm text-muted-foreground mb-4">
                          Transform your style with our best-selling product. Thousands of happy customers can't be wrong! 
                        </p>
                        <div className="flex gap-2">
                          <span className="text-xs px-2 py-1 bg-primary/10 text-primary rounded">Facebook</span>
                          <span className="text-xs px-2 py-1 bg-secondary/10 text-secondary rounded">Instagram</span>
                          <span className="text-xs px-2 py-1 bg-success/10 text-success rounded">TikTok</span>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-gradient-to-r from-primary/10 to-secondary/10 rounded-xl p-6 border border-primary/20">
                    <div className="flex items-center justify-between mb-4">
                      <span className="text-sm font-medium">AI Creative Generation</span>
                      <span className="text-sm text-primary">3 variations ready</span>
                    </div>
                    <div className="w-full bg-background rounded-full h-2 overflow-hidden">
                      <div className="h-full bg-gradient-to-r from-primary to-secondary animate-pulse" style={{ width: '75%' }} />
                    </div>
                  </div>
                </div>
              )}
              ...
              {activeTab === 'schedule' && (
                <div className="animate-fade-in">
                  ...
                  <div className="bg-primary/10 border border-primary/20 rounded-lg p-4 text-center">
                    <p className="text-sm">Drag and drop to schedule your campaigns</p>
                  </div>
                </div>
              )}
              ...
              {activeTab === 'trends' && (
                <div className="space-y-4 animate-fade-in">
                  <div className="grid lg:grid-cols-3 gap-4">
                    {[
                      { trend: 'POV: You found the perfect gift', growth: '+234%', category: 'Lifestyle' },
                      { trend: 'Things that live rent-free', growth: '+189%', category: 'Fashion' },
                      { trend: 'Tell me you need this without...', growth: '+156%', category: 'Beauty' }
                    ].map((item, i) => (
                      <div key={i} className="bg-popover rounded-xl p-4 border border-border hover:border-primary/50 transition-all cursor-pointer">
                        <div className="flex items-start justify-between mb-2">
                          <TrendingUp className="h-4 w-4 text-success" />
                          <span className="text-xs text-success font-medium">{item.growth}</span>
                        </div>
              ...
              {activeTab === 'analytics' && (
                <div className="space-y-6 animate-fade-in">
                  <div className="grid lg:grid-cols-4 gap-4">
                    {[
                      { label: 'ROAS', value: '3.8x', change: '+12%', color: 'success' },
                      { label: 'CTR', value: '2.4%', change: '+8%', color: 'primary' },
                      { label: 'Conversions', value: '1,234', change: '+24%', color: 'secondary' },
                      { label: 'Ad Spend', value: '$4,567', change: '-5%', color: 'success' }
                    ].map((metric, i) => (
                      <div key={i} className="bg-popover rounded-xl p-4 border border-border">
                        <p className="text-sm text-muted-foreground mb-1">{metric.label}</p>
                        <p className="text-2xl font-bold mb-1">{metric.value}</p>
                        <p className={`text-sm ${metric.color === 'success' ? 'text-success' : metric.color === 'primary' ? 'text-primary' : 'text-secondary'}`}>
                          {metric.change}
                        </p>
                      </div>
                    ))}
